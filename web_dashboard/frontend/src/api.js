...
export const api = {
  ...
  zones: {
    ...
    updateSchedule: (id, scheduleId, data) => request(`/zones/${id}/schedules/${scheduleId}`, { method: "PUT", body: JSON.stringify(data) }),
    deleteThreshold: (id, param) => request(`/zones/${id}/thresholds/${param}`, { method: "DELETE" }),
    ...
    layout: (id) => request(`/zones/${id}/layout`),
    mapObjects: (id) => request(`/zones/${id}/objects`),
  ...
  sensors: {
    ...
    readingsSince: (id, hours) => request(`/sensors/${id}/readings?hours=${hours}`),
  },
  executors: {
